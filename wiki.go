...
*/
	...
	//map = new google.maps.Map(document.getElementById('map'), {center: {lat: parseFloat(positionLat), lng: parseFloat(positionLong)});
...
/*
  ...
  {{with .Markers}} {{range .}}
          lats = {{.Latitude}};
          longs = {{.Longitude}};
         // theseLatlng = new google.maps.LatLng(lats, longs);
          theseLatLng = {lat: lats, lng: longs};
           var marker = new google.maps.Marker({
...
func init() {
	positionLat := rootForm.FormValue("positionLat")
	positionLong := rootForm.FormValue("positionLong")
    //resp, err := http.Get("http://amenimaps.com/amenimapi.php?amenity=toilet&mylat=51.50784&mylon=-0.127324&mode=json&name=rara_pirates2&key=9e0aef248ead76faf63f703fd6b8a52c")
	resp, err := http.Get("http://amenimaps.com/amenimapi.php?amenity=toilet&mylat=%.13f&mylon=%.13f&mode=json&name=rara_pirates2&key=9e0aef248ead76faf63f703fd6b8a52c", positionLat, positionLong)
